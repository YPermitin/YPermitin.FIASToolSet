...
* *.nix (на базе Kestrel + реверс-проекси через Apache/Nginx)
...
Настройка *"CORS:AllowedOrigins"* применяется для настройки политики CORS, чтобы разрешить определнным сайтам делать запросы со стороны клиента к API.
...
* Сделать пример использования API в [веб-прилжоении TinyDevTools](https://tinydevtools.ru/fias).
* Добавить поддержку Docker для развертывания приложения.

## Лицензия, благодарности и послесловие

Проект делается на чистом интересе и не преследуюет коммерческих целей. 
